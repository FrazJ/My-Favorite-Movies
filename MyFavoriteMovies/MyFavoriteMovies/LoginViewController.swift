...
class LoginViewController: UIViewController {
    ...
    func loginWithToken(requestToken: String) {
        ...
        /* 2. Build the URL */
        let urlSring = appDelegate.baseURLSecureString + "/authentication/token/validate_with_login" + appDelegate.escapedParameters(methodArguments)
        let url = NSURL(string: urlSring)
        ...
        let task = session.dataTaskWithRequest(request) {(data, response, error) in
            ...
            guard error == nil else {
                dispatch_async(dispatch_get_main_queue()) {
                    self.debugTextLabel.text = "Login failed. (Authenticating token)."
                }
                return
            }
            ...
            /* 6. Use the data! */
            guard let _ = parsedData["success"] as? Bool else {
                print("Cannot find keys 'success' in \(parsedData) ")
                self.debugTextLabel.text = "Login failed. (Authenticating token)."
                return
            }
    ...
    func getSessionID(requestToken: String) {
        ...
        /* 2. Build the URL */
        let urlString = appDelegate.baseURLSecureString + "/authentication/session/new" + appDelegate.escapedParameters(methodArguments)
        let url = NSURL(string: urlString)
        ...
        let task = session.dataTaskWithRequest(request) {(data, response, error) in
            ...
            guard let sessionId = parsedData["session_id"] as? String else {
                print("Cannot find keys 'session_id' in \(parsedData) ")
                self.debugTextLabel.text = "Login failed. (Getting session ID)."
                return
            }
    ...
    func getUserID(session_id : String) {
        ...
        /* 2. Build the URL */
        let urlString = appDelegate.baseURLSecureString + "account" + appDelegate.escapedParameters(methodArguments)
        print(urlString)
        let url = NSURL(string: urlString)!
        ...
        let task = session.dataTaskWithRequest(request) {data, response, error in
            ...
            guard error == nil else {
                dispatch_async(dispatch_get_main_queue()) {
                    self.debugTextLabel.text = "Login failed. (Getting user ID)."
                }
                return
            }
            ...
            guard let userId = parsedData["id"] as? Int else {
                print("Cannot find keys 'id' in \(parsedData) ")
                self.debugTextLabel.text = "Login failed. (Getting user ID)."
                return
            }
